          ...
          <motion.div key="pick" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex-1 relative z-10">
            ...
            <div className="max-w-3xl mx-auto px-8 py-16 text-center">
              <motion.div {...fadeUp(0.1)}>
                ...
                <h1 className="text-3xl font-black text-[#111] mb-2">Choose Your Stack</h1>
                <p className="text-xs text-black/25 mb-10">Pick a language to practice solo coding</p>
              </motion.div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {SKILLS.map((s,i) => (
                  <motion.button key={s.id} {...fadeUp(0.15+i*0.04)} whileHover={{y:-6,boxShadow:`0 20px 40px ${s.color}15`}} whileTap={{scale:0.97}}
                    ...
                    <div className="mt-3 w-full h-1 rounded-full bg-black/[0.03] overflow-hidden">
          ...
          <motion.div key="coding" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex-1 flex flex-col">
            ...
            <div className="bg-white border-b border-black/8 px-5 py-3 flex items-center justify-between flex-shrink-0 shadow-sm">
              <div className="flex items-center gap-3">
                <button onClick={()=>setPhase('pick')} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-black/25 hover:text-black text-[10px] font-bold border border-black/5 hover:border-black/10 transition-all">
              ...
              <div className="flex items-center gap-2.5">
                <button onClick={()=>setShowChat(!showChat)} className={`p-2.5 rounded-xl border transition-all ${showChat?'bg-purple-50 border-purple-300 text-purple-600 shadow-sm':'bg-white border-black/8 text-black/25 hover:text-black/40'}`}>
                  <Bot className="w-4 h-4"/>
                </button>
                <div className="w-px h-5 bg-black/8"/>
                <div className="px-4 py-2 rounded-xl bg-white border border-black/8 font-mono text-sm font-extrabold text-[#222]">
                  <Clock className="w-3.5 h-3.5 inline mr-1.5 text-black/25"/>No Timer
                </div>
            ...
            <AnimatePresence>
              {showEndConfirm && (
                <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[99] flex items-center justify-center bg-black/30 backdrop-blur-sm">
                  <motion.div initial={{scale:0.9}} animate={{scale:1}} className="bg-white rounded-2xl border border-black/5 shadow-2xl p-8 max-w-sm w-full mx-4">
                    ...
                    <p className="text-xs text-center text-black/30 mb-6">Your progress will be saved but you won't earn additional HP.</p>
                    <div className="flex gap-3">
                      <button onClick={()=>setShowEndConfirm(false)} className="flex-1 py-2.5 rounded-xl border border-black/8 text-xs font-bold text-black/30">Cancel</button>
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="w-[340px] flex-shrink-0 border-r border-black/5 bg-white overflow-y-auto p-5">
                <div className="flex gap-2 mb-4">
                  {questions.map((_,qi) => (
                    <button key={qi} onClick={()=>setQIdx(qi)} className={`flex-1 py-2 rounded-lg text-[10px] font-bold border transition-all ${qi===qIdx?'text-white shadow-sm':'bg-white text-black/30 border-black/5'}`}
                ...
                {questions[qIdx] && (
                  <>
                    ...
                    <h3 className="text-base font-black text-[#111] mb-2">{questions[qIdx].title}</h3>
                    <p className="text-xs text-black/35 leading-relaxed mb-5">{questions[qIdx].desc}</p>
                    ...
                    <div className="mb-4">
                      ...
                      {questions[qIdx].testCases.map((tc,ti) => (
                        <div key={ti} className={`flex items-center gap-2 p-2.5 rounded-lg mb-1.5 border text-[10px] ${testResults[qIdx]?.[ti]===true?'bg-green-50 border-green-200':testResults[qIdx]?.[ti]===false?'bg-red-50 border-red-200':'bg-black/[0.01] border-black/5'}`}>
                          {testResults[qIdx]?.[ti]===true?<CheckCircle2 className="w-3 h-3 text-green-500 flex-shrink-0"/>:testResults[qIdx]?.[ti]===false?<XCircle className="w-3 h-3 text-red-500 flex-shrink-0"/>:<div className="w-3 h-3 rounded-full border border-black/10 flex-shrink-0"/>}
              ...
              <div className="flex-1 flex flex-col">
                <div className="bg-[#1E1E2E] px-4 py-2 flex items-center justify-between border-b border-white/5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-white/20 ml-2">solution.{stack==='python'?'py':stack==='cpp'?'cpp':stack==='sql'?'sql':'js'}</span>
                  </div>
                </div>
              ...
              <AnimatePresence>
                {showChat && (
                  <motion.div initial={{width:0,opacity:0}} animate={{width:300,opacity:1}} exit={{width:0,opacity:0}} className="border-l border-black/5 bg-white flex flex-col overflow-hidden">
                    <div className="px-4 py-3 border-b border-black/5 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Bot className="w-4 h-4 text-purple-500"/><span className="text-xs font-bold">AI Assistant</span>
                      </div>
                      <button onClick={()=>setShowChat(false)}><X className="w-3.5 h-3.5 text-black/20"/></button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-3 space-y-2">
                      {chatHistory.map((m,i)=>(
                        <div key={i} className={`p-2.5 rounded-xl text-[11px] leading-relaxed ${m.role==='ai'?'bg-purple-50 text-purple-800':'bg-black/[0.02] text-black/50 ml-4'}`}>{m.text}</div>
                      ))}
                    </div>
                    <div className="p-3 border-t border-black/5 flex gap-2">
                      <input value={chatMsg} onChange={e=>setChatMsg(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')sendChat();}}
                        placeholder="Ask anything..." className="flex-1 px-3 py-2 rounded-lg border border-black/5 text-xs focus:outline-none focus:border-purple-300"/>
        ...
        {phase === 'results' && (
          <motion.div key="results" initial={{opacity:0}} animate={{opacity:1}} className="flex-1 overflow-y-auto p-8 bg-[#F8F9FC] relative z-10">
            ...
            <div className="max-w-lg mx-auto text-center relative z-10">
              ...
              <p className="text-sm text-black/30 mb-8">Great practice on <span className="font-bold" style={{color:m.color}}>{m.name}</span></p>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[{icon:CheckCircle2,val:passed,label:'Passed',c:'#10B981'},{icon:XCircle,val:failed,label:'Failed',c:'#EF4444'},{icon:Trophy,val:`+${hp}`,label:'HP Earned',c:'#F59E0B'}].map((s,i)=>(
                  <motion.div key={s.label} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.3+i*0.1}}
                    className="rounded-2xl p-5 bg-white border-2 shadow-md text-center" style={{borderColor:`${s.c}20`}}>
              ...
              {/* Feedback */}
              <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.8}} className="bg-white rounded-2xl border border-black/5 p-6 mb-6 shadow-sm">
                <p className="text-sm font-bold text-[#222] mb-3">Rate your experience</p>
