      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'pick' && (
          <motion.div key="pick" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex-1 relative z-10">
            <motion.header {...fadeUp(0)} className="sticky top-0 z-20 bg-white/70 backdrop-blur-xl border-b border-black/5 px-8 py-4 flex items-center gap-4">
              <Link href="/student/challenges" className="flex items-center gap-2 px-3 py-2 rounded-xl text-black/30 hover:text-black hover:bg-black/[0.02] transition-all border border-transparent hover:border-black/5">
            ...
            <div className="max-w-3xl mx-auto px-8 py-16 text-center">
              <motion.div {...fadeUp(0.1)}>
                ...
                <h1 className="text-3xl font-black text-[#111] mb-2">Challenge Mode</h1>
                <p className="text-xs text-black/25 mb-3">Race against other coders — same question, fastest wins!</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {SKILLS.map((s,i) => (
                  <motion.button key={s.id} {...fadeUp(0.15+i*0.04)} whileHover={{y:-6,boxShadow:`0 20px 40px ${s.color}15`}} whileTap={{scale:0.97}}
                    ...
                    className="bg-white rounded-2xl border-2 border-black/5 p-6 text-center hover:border-transparent transition-all duration-300 group">
                    <motion.span whileHover={{scale:1.2}} className="text-3xl block mb-3">{s.icon}</motion.span>
                    <p className="text-sm font-bold text-[#222]">{s.name}</p>
                  </motion.button>
                ))}
              </div>
            </div>
        ...
        {phase === 'matching' && (
          <motion.div key="matching" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex-1 flex items-center justify-center relative z-10">
            <div className="text-center">
              <motion.div animate={{rotate:360}} transition={{duration:2,repeat:Infinity,ease:'linear'}} className="w-20 h-20 mx-auto rounded-full border-4 border-black/5 border-t-green-500 mb-8"/>
              <h2 className="text-2xl font-black text-[#111] mb-2">Finding Opponents...</h2>
              <p className="text-xs text-black/25 mb-6">Matching you with coders of similar skill</p>
              <div className="flex items-center justify-center gap-3 mb-8">
                {FAKE_OPPONENTS.map((o,i) => (
                  <motion.div key={o.name} initial={{scale:0,opacity:0}} animate={{scale:1,opacity:1}} transition={{delay:0.5+i*0.3,type:'spring'}}
                    ...
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-black/10 to-black/5 flex items-center justify-center text-xs font-black text-black/30 shadow">{o.avatar}</div>
                    <span className="text-[10px] font-bold text-black/30">{o.name}</span>
                  </motion.div>
                ))}
              </div>
          ...
          <motion.div key="coding" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex-1 flex flex-col">
            ...
            <div className="bg-white border-b border-black/8 px-5 py-3 flex items-center justify-between flex-shrink-0 shadow-sm">
              ...
              <div className="flex items-center gap-2.5">
                {/* Opponents progress */}
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-black/[0.02] border border-black/5">
                  {opponents.map((o,i) => (
                    <div key={o.name} className="flex items-center gap-1.5" title={`${o.name}: ${Math.round(o.progress)}%`}>
                      <div className="w-5 h-5 rounded-md bg-black/10 flex items-center justify-center text-[7px] font-black text-black/30">{o.avatar}</div>
                ...
                <div className="w-px h-5 bg-black/8"/>
                <div className={`px-4 py-2 rounded-xl font-mono text-base font-extrabold border ${timeLeft<60?'bg-red-50 border-red-300 text-red-600 animate-pulse shadow-sm':timeLeft<180?'bg-amber-50 border-amber-200 text-amber-600':'bg-white border-black/8 text-[#222]'}`}>
              ...
              {showEndConfirm && (
                <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[99] flex items-center justify-center bg-black/30 backdrop-blur-sm">
                  <motion.div initial={{scale:0.9,opacity:0}} animate={{scale:1,opacity:1}} className="bg-white rounded-2xl border border-black/5 shadow-2xl p-8 max-w-sm w-full mx-4">
                    ...
                    <h3 className="text-lg font-black text-center text-[#111] mb-2">End Challenge?</h3>
                    <p className="text-xs text-center text-black/30 mb-6">You will forfeit this round. Your current progress will still be saved.</p>
            ...
            <div className="flex-1 flex overflow-hidden">
              {/* LEFT — Question */}
              <div className="w-[340px] flex-shrink-0 border-r border-black/5 bg-white overflow-y-auto p-5">
                {question && (
                  <>
                    ...
                    <h3 className="text-base font-black text-[#111] mb-2">{question.title}</h3>
                    <p className="text-xs text-black/35 leading-relaxed mb-5">{question.desc}</p>

                    {/* My progress bar */}
                    <div className="mb-4 p-3 rounded-xl bg-black/[0.01] border border-black/5">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[9px] font-bold text-black/25">Your Progress</span>
                    ...
                    <p className="text-[9px] font-bold uppercase tracking-wider text-black/20 mb-2">Test Cases</p>
                    {question.testCases.map((tc,ti) => (
                      <div key={ti} className={`flex items-center gap-2 p-2.5 rounded-lg mb-1.5 border text-[10px] ${testResults[ti]===true?'bg-green-50 border-green-200':testResults[ti]===false?'bg-red-50 border-red-200':'bg-black/[0.01] border-black/5'}`}>
                        {testResults[ti]===true?<CheckCircle2 className="w-3 h-3 text-green-500 flex-shrink-0"/>:testResults[ti]===false?<XCircle className="w-3 h-3 text-red-500 flex-shrink-0"/>:<div className="w-3 h-3 rounded-full border border-black/10 flex-shrink-0"/>}
                        <code className="font-mono text-black/40 truncate">{tc.input}</code>
                        <span className="text-black/15 flex-shrink-0">→</span>
                        <code className="font-mono text-black/40 flex-shrink-0">{tc.expected}</code>
                      </div>
                    ))}
                    ...
                    {output && <p className="text-[10px] text-center mt-2 font-bold text-black/30">{output}</p>}
                  </>
                )}
              </div>
              ...
              <div className="flex-1 flex flex-col">
                <div className="bg-[#1E1E2E] px-4 py-2 flex items-center justify-between border-b border-white/5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-white/20 ml-2">challenge.{stack==='python'?'py':stack==='cpp'?'cpp':'js'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <textarea value={code} onChange={e=>setCode(e.target.value)}
                  className="flex-1 bg-[#1E1E2E] text-[#CDD6F4] font-mono text-sm p-5 resize-none focus:outline-none leading-relaxed"
        ...
        {/* ═══ RESULTS ═══ */}
        {phase === 'results' && (
          <motion.div key="results" initial={{opacity:0}} animate={{opacity:1}} className="flex-1 overflow-y-auto p-8 bg-[#F8F9FC] relative z-10">
            ...
            <div className="max-w-lg mx-auto text-center relative z-10">
              ...
              {/* Standings */}
              <motion.div initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} transition={{delay:0.4}} className="bg-white rounded-2xl border border-black/5 shadow-lg p-5 mb-6">
                <p className="text-[9px] font-bold uppercase tracking-widest text-black/20 mb-3">Final Standings</p>
                {[{name:'You',progress:myProgress,isYou:true},...opponents].sort((a,b)=>(b.progress===100?1:0)-(a.progress===100?1:0)||b.progress-a.progress).map((p,i)=>(
                  <div key={p.name} className={`flex items-center gap-3 p-3 rounded-xl mb-1.5 ${(p as any).isYou?'bg-amber-50 border border-amber-200':'bg-black/[0.01]'}`}>
                    <span className="text-sm font-black text-black/20 w-5">{i+1}</span>
                    <span className={`text-xs font-bold ${(p as any).isYou?'text-amber-600':'text-[#222]'}`}>{p.name}</span>
                    <div className="flex-1 h-1.5 rounded-full bg-black/[0.04] overflow-hidden ml-2">
                      <div className="h-full rounded-full" style={{width:`${p.progress}%`,background:p.progress>=100?'#10B981':'#CBD5E1'}}/>
                    </div>
                    <span className="text-[9px] font-bold text-black/20">{Math.round(p.progress)}%</span>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[{icon:CheckCircle2,val:passed,label:'Passed',c:'#10B981'},{icon:XCircle,val:failed,label:'Failed',c:'#EF4444'},{icon:Trophy,val:`+${hp}`,label:'HP',c:'#F59E0B'}].map((s,i)=>(
                  <motion.div key={s.label} initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} transition={{delay:0.6+i*0.1}}
                    className="rounded-2xl p-4 bg-white border-2 shadow-md text-center" style={{borderColor:`${s.c}20`}}>
                    <s.icon className="w-5 h-5 mx-auto mb-1" style={{color:s.c}}/>
                    <p className="text-xl font-black" style={{color:s.c}}>{s.val}</p>
                    <p className="text-[8px] text-black/25 uppercase font-bold tracking-wider">{s.label}</p>
                  </motion.div>
                ))}
              </div>
