...
export default function ChallengesPage() {
  ...
  useEffect(() => {
    try {
      ...
      setHpScore(p.hp||0);
      setXpScore((p.sessions||0) * 50 + (p.hp||0) * 2);
    } catch(_) {}
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FC] relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <motion.div animate={{scale:[1,1.2,1],opacity:[0.02,0.04,0.02]}} transition={{duration:8,repeat:Infinity}} className="absolute top-0 right-1/4 w-[600px] h-[600px] bg-[#FF4D6D] rounded-full blur-[150px]" />
        <motion.div animate={{scale:[1.2,1,1.2],opacity:[0.02,0.04,0.02]}} transition={{duration:10,repeat:Infinity}} className="absolute bottom-0 left-1/3 w-[500px] h-[500px] bg-[#4D79FF] rounded-full blur-[130px]" />
        <motion.div animate={{scale:[1,1.3,1],opacity:[0.02,0.03,0.02]}} transition={{duration:6,repeat:Infinity}} className="absolute top-1/2 right-0 w-[400px] h-[400px] bg-[#10B981] rounded-full blur-[100px]" />
      </div>
      ...
      <motion.header {...fadeUp(0)} className="sticky top-0 z-20 bg-white/70 backdrop-blur-xl border-b border-black/5 px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <motion.div whileHover={{scale:1.05}} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-black/5 shadow-sm">
            <Sparkles className="w-4 h-4 text-amber-500"/>
            <span className="text-sm font-extrabold text-[#222]">{xpScore.toLocaleString()} XP</span>
          </motion.div>
          <motion.div whileHover={{scale:1.05}} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-black/5 shadow-sm">
            <Zap className="w-4 h-4 text-[#FF4D6D]"/>
            <span className="text-sm font-extrabold text-[#222]">{hpScore} HP</span>
          </motion.div>
        </div>
      </motion.header>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-8 py-16">
        ...
        <motion.div {...fadeUp(0.1)} className="text-center mb-14">
          ...
          <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-black/20">Choose your interaction gateway</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {PROTOCOLS.map((p,i) => (
            <motion.div key={p.id} {...fadeUp(0.2+i*0.1)}
              ...
              className="group relative bg-white rounded-2xl border-2 overflow-hidden cursor-pointer"
              style={{
            ...
            >
              ...
              <div className="px-8 pb-8">
                <h3 className="text-xl font-black text-[#111] mb-2 tracking-wide">{p.title}</h3>
                <p className="text-xs text-black/35 leading-relaxed mb-5">{p.desc}</p>
                ...
                <div className="space-y-2 mb-5">
                  {p.features.slice(0,3).map((f,fi) => (
                    <div key={fi} className="flex items-center gap-2.5">
                      ...
                      <span className="text-[10px] text-black/30 font-medium">{f}</span>
                    </div>
                  ))}
                </div>
        ...
        <motion.div {...fadeUp(0.5)} className="flex items-center justify-center gap-6">
          {[{icon:Sparkles,title:'XP COINS',desc:'Used for bio ranking & leaderboards',c:'#F59E0B'},{icon:Shield,title:'HP SCORE',desc:'Honor points for collaboration quality',c:'#FF4D6D'}].map(info=>(
            <motion.div key={info.title} whileHover={{y:-3}} className="flex items-center gap-3 px-6 py-4 rounded-xl bg-white border border-black/5 shadow-sm">
              <info.icon className="w-5 h-5" style={{color:info.c}}/>
              <div>
                <p className="text-xs font-extrabold text-[#222]">{info.title}</p>
                <p className="text-[8px] text-black/20 font-bold uppercase tracking-wider">{info.desc}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
        ...
        {selected && (
          <>
            ...
            <motion.div initial={{opacity:0,y:40,scale:0.95}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:40,scale:0.95}} transition={{type:'spring',damping:25,stiffness:300}}
              ...
              <div className="bg-white rounded-3xl border border-black/5 shadow-2xl max-w-lg w-full overflow-hidden">
                {/* Header */}
                <div className="relative p-8 pb-6" style={{background:`linear-gradient(135deg,${selected.color}08,${selected.color}03)`}}>
                  <button onClick={()=>setSelectedProtocol(null)} className="absolute top-6 right-6 w-8 h-8 rounded-xl bg-black/5 flex items-center justify-center hover:bg-black/10 transition-colors">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-[#111]">{selected.title}</h3>
                      <p className="text-[10px] text-black/25 uppercase tracking-widest font-bold">How it works</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-8 py-6">
                  <div className="space-y-4 mb-6">
                    {selected.steps.map((step,i) => (
                      <motion.div key={i} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:0.1+i*0.1}}
                        ...
                        <div className="pb-2">
                          <p className="text-sm font-bold text-[#111] mb-0.5">{step.s}</p>
                          <p className="text-[11px] text-black/30 leading-relaxed">{step.d}</p>
                        </div>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-3">
                    <button onClick={()=>setSelectedProtocol(null)} className="flex-1 py-3.5 rounded-xl border border-black/8 text-xs font-bold text-black/30 hover:text-black transition-all">Cancel</button>
                    <motion.button whileHover={{scale:1.03}} whileTap={{scale:0.97}} onClick={()=>handleStart(selected.id)}
                      ...
                      {selected.id==='partner'?'Start Assessment':selected.id==='solo'?'Start Coding':'Enter Challenge'}
