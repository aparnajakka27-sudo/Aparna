...
export default function MentorProfileSetup() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col relative overflow-hidden">
      ...
      {/* Header */}
      <header className="relative z-10 px-8 py-6 border-b border-black/5 bg-white/70 backdrop-blur-xl flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold text-black/30">STEP {step}/3</span>
        </div>
      </header>
      ...
      <div className="flex-1 relative z-10 flex items-center justify-center p-8">
        <div className="max-w-xl w-full">
          {step === 1 && (
            <motion.div key="step1" {...fadeUp(0)} className="bg-white rounded-3xl border border-black/5 shadow-2xl p-10">
              ...
              <h1 className="text-3xl font-black text-[#111] mb-2">Welcome, Mentor!</h1>
              <p className="text-sm text-black/40 mb-8">Let's set up your profile so students can find you for guidance.</p>

              <div className="space-y-6 mb-8">
                <div>
                  <label className="block text-xs font-bold text-[#222] mb-2">Full Name</label>
                  <input type="text" value={name} onChange={e=>setName(e.target.value)} placeholder="E.g. Dr. Priya Patel"
                    className="w-full px-4 py-3 rounded-xl bg-black/[0.02] border border-black/5 focus:border-[#A855F7] focus:ring-4 focus:ring-[#A855F7]/10 outline-none text-sm font-bold text-[#222] transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-[#222] mb-2">Professional Title</label>
                  <input type="text" value={title} onChange={e=>setTitle(e.target.value)} placeholder="E.g. Senior Software Engineer @ Google"
                    className="w-full px-4 py-3 rounded-xl bg-black/[0.02] border border-black/5 focus:border-[#A855F7] focus:ring-4 focus:ring-[#A855F7]/10 outline-none text-sm font-bold text-[#222] transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-[#222] mb-3">Choose Avatar</label>
                  <div className="flex gap-3">
                    {AVATARS.slice(0,6).map(a => (
                      <button key={a} onClick={()=>setAvatar(a)} className={`w-12 h-12 rounded-2xl text-2xl flex items-center justify-center transition-all ${avatar===a?'bg-purple-50 border-2 border-purple-400 scale-110 shadow-lg':'bg-black/[0.02] border border-black/5 hover:bg-black/[0.05]'}`}>
                        {a}
                      </button>
                    ))}
                  </div>
          ...
          {step === 2 && (
            <motion.div key="step2" {...fadeUp(0)} className="bg-white rounded-3xl border border-black/5 shadow-2xl p-10">
              ...
              <h1 className="text-3xl font-black text-[#111] mb-2">Your Tech Stack</h1>
              <p className="text-sm text-black/40 mb-8">What languages and frameworks are you proficient in? (Max 5)</p>
                ...
                {SKILLS.map(skill => {
                  ...
                  return (
                    <button key={skill} onClick={()=>toggleSkill(skill, skillsKnown, setSkillsKnown)}
                      className={`px-5 py-3 rounded-xl text-xs font-bold transition-all border-2 ${isSelected?'bg-cyan-50 border-cyan-400 text-cyan-600 shadow-sm scale-105':'bg-white border-black/5 text-black/30 hover:border-black/10 hover:text-black'}`}>
              ...
              <div className="flex gap-4">
                <button onClick={()=>setStep(1)} className="px-6 py-4 rounded-xl border border-black/10 text-xs font-bold text-black/40 hover:text-black">Back</button>
          ...
          {step === 3 && (
            <motion.div key="step3" {...fadeUp(0)} className="bg-white rounded-3xl border border-black/5 shadow-2xl p-10">
              ...
              <h1 className="text-3xl font-black text-[#111] mb-2">What will you teach?</h1>
              <p className="text-sm text-black/40 mb-8">Select the specific areas you want to guide students in.</p>
                ...
                {skillsKnown.map(skill => {
                  ...
                  return (
                    <button key={skill} onClick={()=>toggleSkill(skill, skillsGuide, setSkillsGuide)}
                      className={`px-5 py-3 rounded-xl text-xs font-bold transition-all border-2 ${isSelected?'bg-green-50 border-green-400 text-green-600 shadow-sm scale-105':'bg-white border-black/5 text-black/30 hover:border-black/10 hover:text-black'}`}>
                      {skill} {isSelected && <Check className="w-3 h-3 inline ml-1"/>}
                    </button>
                  );
                })}
              ...
              <div className="flex gap-4">
                <button onClick={()=>setStep(2)} className="px-6 py-4 rounded-xl border border-black/10 text-xs font-bold text-black/40 hover:text-black">Back</button>
                <button disabled={skillsGuide.length===0 || loading} onClick={finishSetup} className="flex-1 py-4 rounded-xl text-white text-sm font-black flex items-center justify-center gap-2 disabled:opacity-50 shadow-xl"
                  ...
                  {loading ? <motion.div animate={{rotate:360}} transition={{duration:1,repeat:Infinity,ease:'linear'}} className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full"/> : <><Sparkles className="w-4 h-4"/> Complete Setup</>}
                </button>
              </div>
            </motion.div>
          )}
